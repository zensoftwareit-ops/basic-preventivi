...
function render_header(string $title, string $active = 'dashboard'): void
{
    ...
    $deviceStatus = Auth::persistentDeviceActive()
        ? '<small class="device-login-status active">● Accesso dispositivo attivo</small>'
        : '<small class="device-login-status">Accesso dispositivo non persistente</small>';
    echo '</nav><div class="sidebar-tools">' . $pairControl . '<button class="sidebar-tool" type="button" data-install-app hidden>Installa app</button>' . $pushControl . $deviceStatus . '<small data-pwa-status aria-live="polite"></small></div><div class="sidebar-footer"><span class="avatar">' . e(mb_strtoupper(mb_substr($user['display_name'] ?? 'U', 0, 1))) . '</span><div><strong>' . e($user['display_name'] ?? '') . '</strong><small>' . e($roleLabel) . '</small></div><a class="logout" href="' . e(url('logout.php')) . '" title="Esci">Esci</a></div></aside>';
