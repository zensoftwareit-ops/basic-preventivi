...
function render_header(string $title, string $active = 'dashboard'): void
{
    ...
    echo '<link rel="stylesheet" href="assets/app.css?v=20260717d"></head><body data-push-public-key="' . e($pushPublicKey) . '" data-mobile-setup="' . ($mobileSetup ? '1' : '0') . '">';
    ...
    $pairControl = '<button class="sidebar-tool" type="button" data-mobile-pair>Collega smartphone · QR</button>';
    ...
    echo '<div class="app-shell"><header class="topbar"><button class="menu-button" type="button" data-menu aria-label="Apri menu">☰</button><div><h1>' . $titleSafe . '</h1><p>' . e((new DateTimeImmutable())->format('d/m/Y')) . '</p></div><button class="button secondary compact topbar-pair" type="button" data-mobile-pair>Collega smartphone</button><a class="button primary compact" href="' . e(url('index.php', ['page' => 'quote_new'])) . '">+ Nuova richiesta</a></header><main class="content">';
...
{
    $canReceivePush = (string) (Auth::user()['role'] ?? 'operator') !== 'super';
    $pushStep = $canReceivePush
        ? '<li><span>2</span><div><strong>Attiva le notifiche</strong><small>Riceverai soltanto le scadenze dei preventivi assegnati a te.</small></div></li>'
        : '';
    $pushSetupButton = $canReceivePush
        ? '<button class="button secondary" type="button" data-mobile-setup-push disabled>Attiva notifiche</button>'
        : '';
    $setupActionsClass = $canReceivePush ? 'setup-actions' : 'setup-actions single';
    ...
    echo '<script src="assets/vendor/qrcode.js?v=df13f829"></script><script src="assets/app.js?v=20260717d"></script></body></html>';
}
